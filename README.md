...
- URL fetching is restricted to public `http`/`https`, rejects embedded credentials, blocks private IP ranges, limits redirects, and caps response size.
- Atlassian integration accepts only Atlassian Cloud HTTPS hostnames ending in `.atlassian.net`, for example `company.atlassian.net`, not full ticket/wiki URLs.
